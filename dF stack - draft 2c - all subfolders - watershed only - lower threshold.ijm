...
blur = 3; //Gaussian blur to solidify waves and smooth individual cell details
threshold = 1.03; //dF threshold for retinal wave
  ...
  for (i=0; i<file_list.length; i++) {
      if(endsWith(file_list[i], "/"))
      	...
      	for(a=0; a<watershed_file_list.length; a++){
      		if(matches(watershed_file_list[a], "watershed low - " + file_prefix + ".tif")){
...
function processFile(file_prefix, out_dir) {
	...
	close("\\Others");
	saveAs("tiff", out_dir + "watershed low - " + file_prefix + ".tif");
	rename("watershed");
