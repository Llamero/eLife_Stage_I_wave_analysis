...
function processFiles(dir, out_dir) {
  ...
  for (i=0; i<file_list.length; i++) {
      if(endsWith(file_list[i], "/"))
      ...
      else if(startsWith(file_list[i], "watershed low - ") && endsWith(file_list[i], "tif")){
      	close("*");
      	run("Collect Garbage");
      	...
      	print(dir);
      	file_prefix = replace(file_list[i], "watershed low - ", "");
      	...
      	for(a=0; a<watershed_file_list.length; a++){
      		if(matches(watershed_file_list[a], "histogram low -" + file_prefix + ".tif")){
...
function processFile(file_prefix, out_dir) {
	...
	close("\\Others");
	saveAs("tiff", out_dir + "histogram low -" + file_prefix + ".tif");
